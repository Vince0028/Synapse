...
const PixelCat = ({ frame, direction, colors, rotation }) => {
    const isStep = frame % 2 === 0;
    return (<svg width="18" height="14" viewBox="0 0 24 20" style={{
...
const PixelDog = ({ frame, direction, colors, rotation }) => {
    const isStep = frame % 2 === 0;
    return (<svg width="20" height="14" viewBox="0 0 28 20" style={{
            transform: `rotate(${rotation}deg) scaleX(${direction === 'left' ? -1 : 1})`,
        }}>
...
// Interaction emojis
const InteractionEmoji = ({ x, y, type }) => (<div className="absolute pointer-events-none animate-bounce text-sm" style={{ left: x, top: y - 20, zIndex: 100 }}>
    {type}
  </div>);
const INTERACTION_EMOJIS = ['❤️', '💕', '💖', '✨', '💫', '🐾', '💗', '😻', '🥰', '💞'];
const createRandomPet = (id) => {
...
export function HeaderPet() {
    ...
    const getPositionFromPerimeter = (perimeterPos) => {
    ...
    const checkInteractions = (currentPets) => {
    ...
    useEffect(() => {
        const walkInterval = setInterval(() => {
            ...
            // Decay interactions
            setInteractions(prev => prev.map(i => ({ ...i, ttl: i.ttl - 1 })).filter(i => i.ttl > 0));
        }, 80);
        return () => clearInterval(walkInterval);
    }, [dimensions]);
    return (<>
      {/* Interactions */}
      {interactions.map(interaction => (<InteractionEmoji key={interaction.id} x={interaction.x} y={interaction.y} type={interaction.emoji}/>))}
      ...
      {pets.map(pet => {
            ...
            const visualDirection = pet.direction === 1 ? 'right' : 'left';
            return (<div key={pet.id} className="absolute transition-none pointer-events-none select-none" style={{
                    left: x,
                    top: y,
                    zIndex: 50 + pet.id,
                }}>
            {pet.type === 'cat' ? (<PixelCat frame={pet.frame} direction={visualDirection} colors={pet.colors} rotation={rotation}/>) : (<PixelDog frame={pet.frame} direction={visualDirection} colors={pet.colors} rotation={rotation}/>)}
          </div>);
        })}
    </>);
}

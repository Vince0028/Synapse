...
export function FloatingTagCloud({ tags, selectedTag, onTagClick }) {
    ...
    const getSizeClass = (count) => {
        if (max === min)
            return "text-sm p-4"; // Default if all same
        // Calculate percentile or relative size
        const relative = (count - min) / (max - min); // 0 to 1
        // Reduced sizes as requested ("too big")
        if (relative > 0.8)
            return "text-xl p-6 font-bold z-20 shadow-xl"; // Top 20% - Big Bubble
        if (relative > 0.6)
            return "text-lg p-5 font-semibold z-10 shadow-lg";
        if (relative > 0.4)
            return "text-base p-4 font-medium z-10";
        if (relative > 0.2)
            return "text-sm p-3 font-medium z-0";
        return "text-xs p-2.5 z-0"; // Smallest
    };
    return (<div className="flex flex-wrap items-center justify-center gap-4 p-8 min-h-[300px] w-full max-w-6xl mx-auto perspective-1000">
            {tags.map((tag, index) => {
            ...
            return (<button key={tag.name} onClick={() => onTagClick(tag.name)} style={{
                    animationDelay,
                    animationDuration,
                }} className={cn("rounded-full backdrop-blur-md transition-all duration-300 ease-out", "border shadow-lg hover:scale-110", "animate-float", getSizeClass(tag.count), isSelected

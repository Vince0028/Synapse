...
import { LayoutGrid, GraduationCap, Palette, Sparkles, Code2, ChevronRight, Bot, UserCheck, Terminal, Cpu, Server, Database, Wrench, BookOpen, PenTool, Video, Newspaper, Film, Globe, FileText, Workflow, Cloud, Box } from "lucide-react";
import { Sidebar, SidebarContent, SidebarHeader, SidebarMenu, SidebarMenuButton, SidebarMenuItem, SidebarRail, SidebarMenuSub, SidebarMenuSubItem, SidebarMenuSubButton, } from "@/components/ui/sidebar";
...
export function AppSidebar({ activeCategory, onCategoryChange, ...props }) {
    const [openGroup, setOpenGroup] = React.useState(() => {
        const found = sidebarStructure.find(group => group.type === "group" && group.items?.some(item => item.id === activeCategory));
    ...
    return (<Sidebar {...props}>
            ...
            if (group.type === "single") {
                ...
                return (<SidebarMenuItem key={group.id || index}>
                                    <SidebarMenuButton isActive={isActive} onClick={() => {
            ...
            return (<Collapsible key={group.id} open={isGroupOpen} onOpenChange={(open) => {
                    if (open) {
                    ...
                    return (<SidebarMenuSubItem key={item.id}>
                                                        <SidebarMenuSubButton isActive={isActive} onClick={() => {

...
function PromptCard({ prompt }) {
    ...
    return (<div className={cn("relative rounded-sm p-6", "bg-card border border-border", "shadow-card", "font-mono flex flex-col h-[400px]", "transition-all duration-300 hover:shadow-hover hover:-translate-y-1", "before:content-[''] before:absolute before:top-0 before:bottom-0 before:left-8 before:w-px before:bg-primary/5")}>
      ...
      <button onClick={handleCopy} className={cn("mt-auto ml-6 py-2 px-4 rounded-md", "bg-secondary hover:bg-secondary/80 text-secondary-foreground", "border border-border", "font-semibold text-xs uppercase tracking-wider", "flex items-center justify-center gap-2", "transition-colors shadow-sm", "relative z-10")}>
        {copied ? (<>

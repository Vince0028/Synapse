      ...
      <SidebarInset>
        <div className="min-h-screen transition-colors duration-300">
          ...
          <main className="container mx-auto px-6 py-8 space-y-8 pb-32">
              ...
              {activeCategory !== "prompts" && (<>
                  <div className="max-w-2xl mx-auto space-y-4">
                    ...
                    <div className="flex flex-wrap gap-2 justify-center">
                      <button onClick={() => setPricingFilter(pricingFilter === "Free" ? null : "Free")} className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors border ${pricingFilter === "Free"
                ? "bg-green-100 text-green-700 border-green-200 dark:bg-green-900/30 dark:text-green-300 dark:border-green-800"
                : "bg-secondary hover:bg-secondary/80 text-secondary-foreground border-transparent"}`}>
                        Free Only
                      </button>
                      <button onClick={() => setPricingFilter(pricingFilter === "Paid" ? null : "Paid")} className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors border ${pricingFilter === "Paid"
                ? "bg-blue-100 text-blue-700 border-blue-200 dark:bg-blue-900/30 dark:text-blue-300 dark:border-blue-800"
                : "bg-secondary hover:bg-secondary/80 text-secondary-foreground border-transparent"}`}>
                        Paid / Freemium
                      </button>
                      <button onClick={() => setStudentFilter(!studentFilter)} className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors border ${studentFilter
                ? "bg-indigo-100 text-indigo-700 border-indigo-200 dark:bg-indigo-900/30 dark:text-indigo-300 dark:border-indigo-800"
                : "bg-secondary hover:bg-secondary/80 text-secondary-foreground border-transparent"}`}>
                        Student Offers
                      </button>
                      <button onClick={() => setFavoritesFilter(!favoritesFilter)} className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors border flex items-center gap-1.5 ${favoritesFilter
                ? "bg-yellow-100 text-yellow-700 border-yellow-200 dark:bg-yellow-900/30 dark:text-yellow-300 dark:border-yellow-800"
                : "bg-secondary hover:bg-secondary/80 text-secondary-foreground border-transparent"}`}>
                        <Star className={`w-3.5 h-3.5 ${favoritesFilter ? "fill-current" : ""}`}/>
                        Favorites
                        {getFavoriteCount() > 0 && (<span className="ml-1 px-1.5 py-0.5 text-xs rounded-full bg-yellow-500 text-white">
                            {getFavoriteCount()}
                          </span>)}
                      </button>
                    </div>
                  ...
                  {/* Dynamic Tag Bubbles */}
                  {availableTags.length > 0 && (<FloatingTagCloud tags={availableTags} selectedTag={tagFilter} onTagClick={(tag) => setTagFilter(tagFilter === tag ? null : tag)}/>)}
                </>)}
            </div>

            {/* Category Header */}
            <CategoryHeader activeCategory={activeCategory} title={categoryLabels[activeCategory] || "Resources"} resultCount={activeCategory === "prompts" ? undefined : filteredLinks.length}/>

            {/* Bento Grid (Hide if on Prompts tab) */}
            {activeCategory !== "prompts" && (<div key={`grid-${activeCategory}-${tagFilter || 'all'}`} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {filteredLinks.map((link, index) => (<LinkCard key={`${activeCategory}-${link.id}`} link={link} index={index} isFavorite={isFavorite(link.id)} onToggleFavorite={toggleFavorite}/>))}

                {filteredLinks.length === 0 && (<div className="col-span-full text-center py-16">
            ...
            {(activeCategory === "prompts" || (activeCategory === "all" && !favoritesFilter && !tagFilter)) && (<section className={activeCategory === "all" ? "max-w-full pt-8 border-t" : "max-w-full"}>
                {activeCategory === "all" && (<h2 className="text-2xl font-bold text-foreground mb-6">
                    Prompt Vault
                  </h2>)}
                <PromptVault />
              </section>)}
